...
def _regular_strategy(
    ...
    *,
    ...
    g1_stats: dict[int, int] = {}
    for v, _ in g1_pq:
        if v not in g1_stats:
            g1_stats[v] = 0
        g1_stats[v] += 1
    g2_stats: dict[int, int] = {}
    for v, _ in g2_pq:
        if v not in g2_stats:
            g2_stats[v] = 0
        g2_stats[v] += 1
    ...
    while len(g2_pq) > 0:
        ...
        g1_stats_t = tqdm(
            ...
            initial=g1_stats.get(max_weight, 0),
            total=g1_stats.get(max_weight, 0),
            leave=False,
            disable=quiet,
            position=1,
            unit="",
        ...
        while len(g1_pq) > 0 and g1_pq[0][0] <= max_weight:
            ...
            for atomic_sig, atomic_w in g1_atomic_lookup.items():
                comb_w = atomic_w + w
                heapq.heappush(g1_pq, (comb_w, atomic_sig ^ sig))
                g1_stats[comb_w] = g1_stats.get(comb_w, 0) + 1
                if comb_w == max_weight:
                    g1_stats_t.update(n=1)
        g1_stats_t.close()
        ...
        g2_stats_t = tqdm(
            ...
            initial=g2_stats.get(max_weight, 0),
            total=g2_stats.get(max_weight, 0),
            leave=False,
            disable=quiet,
            position=1,
            unit="",
        ...
        while len(g2_pq) > 0 and g2_pq[0][0] <= max_weight:
            ...
            for atomic_sig, atomic_w in g2_atomic_lookup.items():
                comb_w = atomic_w + w
                heapq.heappush(g2_pq, (comb_w, atomic_sig ^ sig))
                g2_stats[comb_w] = g2_stats.get(comb_w, 0) + 1
                if comb_w == max_weight:
                    g2_stats_t.update(n=1)
        g2_stats_t.close()
